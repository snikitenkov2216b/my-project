...
class Category14Tab(QWidget):
    ...
    def __init__(self, calculator: Category14Calculator, parent=None):
        ...
        self.enterprise_inputs, self.enterprise_outputs, self.enterprise_stocks = (
            [],
            [],
            [],
        )
    ...
    def _init_ui(self):
        ...
        self.method_combobox.setToolTip(
            "Выберите метод: по каждому процессу отдельно или по общему балансу предприятия."
        )
    ...
    def _add_process_raw_row(self):
        self._create_dynamic_row(
            self.process_raw,
            self.process_raw_layout,
            self._get_all_materials(),
            [("consumption", "Расход, т", (0.0, 1e9, 6))],
        )

    def _add_process_fuel_row(self):
        self._create_dynamic_row(
            self.process_fuels,
            self.process_fuels_layout,
            self._get_all_materials(),
            [("consumption", "Расход", (0.0, 1e9, 6))],
        )

    def _add_process_product_row(self):
        self._create_dynamic_row(
            self.process_products,
            self.process_products_layout,
            self._get_all_materials(),
            [("production", "Производство, т", (0.0, 1e9, 6))],
        )

    def _add_process_byproduct_row(self):
        self._create_dynamic_row(
            self.process_byproducts,
            self.process_byproducts_layout,
            self._get_all_materials(),
            [("production", "Выход, т", (0.0, 1e9, 6))],
        )

    def _add_enterprise_input_row(self):
        self._create_dynamic_row(
            self.enterprise_inputs,
            self.enterprise_inputs_layout,
            self._get_all_materials(),
            [("mass", "Масса, т", (0.0, 1e9, 6))],
        )

    def _add_enterprise_output_row(self):
        self._create_dynamic_row(
            self.enterprise_outputs,
            self.enterprise_outputs_layout,
            self._get_all_materials(),
            [("mass", "Масса, т", (0.0, 1e9, 6))],
        )

    def _add_enterprise_stock_row(self):
        self._create_dynamic_row(
            self.enterprise_stocks,
            self.enterprise_stocks_layout,
            self._get_all_materials(),
            [("mass_change", "Изменение массы, т", (-1e9, 1e9, 6))],
        )
        ...
        try:
            ...
            if method_index == 0:
                ...
                fuels = [
                    ...
                    for r in self.process_fuels
                ]
                products = [
                    {
                        "name": r["combo"].currentText(),
                        "production": self._get_float(r["production"], "Производство"),
                    }
                    for r in self.process_products
                ]
                by_products = [
                    {
                        "name": r["combo"].currentText(),
                        "production": self._get_float(r["production"], "Выход"),
                    }
                    for r in self.process_byproducts
                ]
                if not raw and not fuels:
                    raise ValueError("Добавьте хотя бы один входящий поток.")
                    co2_emissions = self.calculator.calculate_emissions_by_process(
                        raw, fuels, products, by_products
                    )
            elif method_index == 1:
                inputs = [
                    {
                    ...
                    for r in self.enterprise_inputs
                ]
                outputs = [
                    {
                        "name": r["combo"].currentText(),
                    ...
                    for r in self.enterprise_outputs
                ]
                stock_changes = [
                    {
                        "name": r["combo"].currentText(),
                        "mass_change": self._get_float(r["mass_change"], "Изменение"),
                    }
                    for r in self.enterprise_stocks
                ]
                if not inputs:
                    raise ValueError("Добавьте хотя бы один входящий поток.")
                    co2_emissions = (
                        self.calculator.calculate_emissions_by_enterprise_balance(
                            inputs, outputs, stock_changes
                        )
                    )

            self.result_label.setText(f"Результат: {co2_emissions:.4f} тонн CO2")
            logging.info(f"Category 14 calculation successful: CO2={co2_emissions}")
        except ValueError as e:
            logging.error(f"Category 14 Calculation - ValueError: {e}")
            QMessageBox.warning(self, "Ошибка ввода", str(e))
            self.result_label.setText("Результат: Ошибка")
        except Exception as e:
            logging.critical(
                f"Category 14 Calculation - Unexpected error: {e}", exc_info=True
            )
