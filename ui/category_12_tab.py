...
class Category12Tab(TabDataMixin, QWidget):
    ...
    def _init_ui(self):
        ...
        self.method_combobox.setToolTip(
            "Выберите метод расчета: по общему балансу углерода или как сумму выбросов от отдельных источников."
        )
    ...
    def _add_balance_raw_row(self):
        self._create_dynamic_row(
            self.balance_raw_materials,
            self.balance_raw_layout,
            self.calculator.data_service.get_petrochemical_substance_names_12_1(),
    ...
    def _add_source_cat3_row(self):
        self._create_dynamic_row(
            self.source_fugitive_gases,
            self.source_cat3_layout,
            self.calculator.data_service.get_fugitive_gas_types_table_3_1(),
            [("volume", "Объем, тыс. м3", (0.0, 1e9, 6))],
        )
    ...
    def _add_source_cat1_row(self):
        ...
        self.source_cat1_layout.insertWidget(
            self.source_cat1_layout.count() - 1, row_widget
        )
        remove_button.clicked.connect(
            lambda: self._remove_row(
                row_data, self.source_cat1_layout, self.source_stationary_fuels
            )
        )
    ...
    def _add_source_cat2_row(self):
        ...
        unit_combo.addItems(["тонна", "тыс. м3"])
        remove_button = QPushButton("Удалить")
        layout.addWidget(combo)
        layout.addWidget(consumption)
        ...
        self.source_cat2_layout.insertWidget(
            self.source_cat2_layout.count() - 1, row_widget
        )
        remove_button.clicked.connect(
            lambda: self._remove_row(
                row_data, self.source_cat2_layout, self.source_flare_gases
            )
        )
        ...
        try:
            ...
            if method_index == 0:
                raw = [
                    ...
                    for r in self.balance_raw_materials
                ]
                primary = [
                    {
                        "name": r["combo"].currentText(),
                    ...
                    for r in self.balance_primary_products
                ]
                by_prod = [
                    {
                        "name": r["combo"].currentText(),
                    ...
                    for r in self.balance_by_products
                ]
                if not raw:
                    raise ValueError("Добавьте сырье.")
                ...
                stationary = [
                    {
                        "fuel_name": r["combo"].currentText(),
                        "fuel_consumption": self._get_float(r["consumption"], "Расход"),
                        "oxidation_factor": self._get_float(
                            r["oxidation"], "Коэф. окисления"
                        ),
                    }
                    for r in self.source_stationary_fuels
                ]
                flare = [
                    {
                        "gas_type": r["combo"].currentText(),
                        "consumption": self._get_float(r["consumption"], "Расход"),
                        "unit": r["unit"].currentText(),
                    }
                    for r in self.source_flare_gases
                ]
                fugitive = [
                    {
                        "gas_type": r["combo"].currentText(),
                        "volume": self._get_float(r["volume"], "Объем"),
                    }
                    for r in self.source_fugitive_gases
                ]
                co2_emissions = (
                    self.calculator.calculate_emissions_by_source_categories(
                        stationary, flare, fugitive
                    )
                )

            self.result_label.setText(f"Результат: {co2_emissions:.4f} тонн CO2")
            logging.info(f"Category 12 calculation successful: CO2={co2_emissions}")
            ...
            logging.critical(
                f"Category 12 Calculation - Unexpected error: {e}", exc_info=True
            )
