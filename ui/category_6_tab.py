...
class Category6Tab(TabDataMixin, QWidget):
    """
    ...
    def __init__(self, calculator: Category6Calculator, parent=None):
        ...
        self.clinker_dust_oxide_rows, self.clinker_non_carbonate_rows = [], []
    ...
    def _init_ui(self):
        ...
        self.method_combobox.setToolTip(
            "Выберите метод в зависимости от доступных данных: по количеству потребленного сырья или по количеству произведенного клинкера."
        )
    ...
    def _create_raw_materials_widget(self):
        ...
        self.raw_dust_mass_input.setToolTip(
            "Масса цементной пыли, уловленной и не возвращенной в печь."
        )
        ...
        self.raw_dust_calc_degree_input.setToolTip(
            "Степень кальцинирования карбонатов в пыли (доля от 0 до 1)."
        )
        dust_form.addRow(
            "Степень кальцинирования пыли (доля):", self.raw_dust_calc_degree_input
        )
        ...
        add_dust_carb_btn.setToolTip(
            "Добавьте состав карбонатов в пыли, если он известен."
        )
    ...
    def _create_clinker_widget(self):
        ...
        self.clinker_production_input.setToolTip(
            "Общая масса произведенного клинкера за год."
        )
        self.clinker_cao_fraction_input = self._create_line_edit(
            "", (0.0, 1.0, 4), "Напр., 0.65"
        )
        self.clinker_cao_fraction_input.setToolTip(
            "Массовая доля оксида кальция (CaO) в клинкере."
        )
        self.clinker_mgo_fraction_input = self._create_line_edit(
            "", (0.0, 1.0, 4), "Напр., 0.02"
        )
        self.clinker_mgo_fraction_input.setToolTip(
            "Массовая доля оксида магния (MgO) в клинкере."
        )
        clinker_layout.addRow("Масса клинкера (т):", self.clinker_production_input)
        clinker_layout.addRow("Доля CaO в клинкере:", self.clinker_cao_fraction_input)
        clinker_layout.addRow("Доля MgO в клинкере:", self.clinker_mgo_fraction_input)
        ...
        self.clinker_dust_mass_input.setToolTip(
            "Масса цементной пыли, уловленной и не возвращенной в печь."
        )
        ...
        add_dust_oxide_btn.setToolTip(
            "Добавьте оксидный состав пыли, если он известен."
        )
        ...
        non_carb_group = QGroupBox("Некарбонатное сырье")
    ...
    def _add_raw_dust_carbonate_row(self):
        self._create_dynamic_row(
            self.raw_dust_carbonate_rows,
            self.raw_dust_carbonates_layout,
            self.calculator.data_service.get_carbonate_formulas_table_6_1(),
    ...
    def _add_raw_non_carbonate_row(self):
        self._create_dynamic_row(
            self.raw_non_carbonate_rows,
            self.raw_non_carbonates_layout,
    ...
    def _add_clinker_dust_oxide_row(self):
        self._create_dynamic_row(
            ...
            self.clinker_dust_oxides_layout,
            ["CaO", "MgO"],
            [("fraction", "Доля в пыли", (0.0, 1.0, 4))],
        )
    ...
    def _add_clinker_non_carbonate_row(self):
        self._create_dynamic_row(
            ...
            self.clinker_non_carbonates_layout,
            self.calculator.data_service.get_fuels_table_1_1(),
            [("consumption", "Расход, т", (0.0, 1e9, 6))],
        )
        ...
        try:
            ...
            if current_method_index == 0:
                carbonates = [
                    {
                        ...
                        "mass": self._get_float(r["mass"], "Масса"),
                        "calcination_degree": self._get_float(
                            r["calc_degree"], "Степень кальц."
                        ),
                    }
                    for r in self.raw_carbonate_rows
                ]
                non_carbonates = [
                    {
                        "name": r["combo"].currentText(),
                        "consumption": self._get_float(r["consumption"], "Расход"),
                    }
                    for r in self.raw_non_carbonate_rows
                ]
                dust_mass = self._get_float(self.raw_dust_mass_input, "Масса пыли")
                dust_calc_degree = self._get_float(
                    self.raw_dust_calc_degree_input, "Степень кальц. пыли"
                )
                dust_fractions = [
                    {
                        "name": r["combo"].currentText(),
                        "fraction": self._get_float(r["fraction"], "Доля в пыли"),
                    }
                    for r in self.raw_dust_carbonate_rows
                ]
                cement_dust = {
                    "mass": dust_mass,
                    "calcination_degree": dust_calc_degree,
                    "carbonate_fractions": dust_fractions,
                }
                if not carbonates:
                ...
                co2_emissions = self.calculator.calculate_based_on_raw_materials(
                    carbonates, cement_dust, non_carbonates
                )
            elif current_method_index == 1:
                clinker_prod = self._get_float(
                    self.clinker_production_input, "Масса клинкера"
                )
                ...
                clinker_comp = [
                    {"oxide_name": "CaO", "fraction": cao_frac},
                    {"oxide_name": "MgO", "fraction": mgo_frac},
                ]
                dust_mass = self._get_float(self.clinker_dust_mass_input, "Масса пыли")
                dust_oxides = [
                    {
                        "oxide_name": r["combo"].currentText(),
                        "fraction": self._get_float(r["fraction"], "Доля оксида"),
                    }
                    for r in self.clinker_dust_oxide_rows
                ]
                ...
                non_carbonates = [
                    ...
                    for r in self.clinker_non_carbonate_rows
                ]
                co2_emissions = self.calculator.calculate_based_on_clinker(
                    clinker_prod, clinker_comp, cement_dust, non_carbonates
                )

            self.result_label.setText(f"Результат: {co2_emissions:.4f} тонн CO2")
            logging.info(f"Category 6 calculation successful: CO2={co2_emissions}")
            ...
            logging.critical(
                f"Category 6 Calculation - Unexpected error: {e}", exc_info=True
            )
