    ...
    def _perform_calculation(self):
        ...
        try:
            ...
            for i, row in enumerate(self.raw_material_rows):
                ...
                fraction = self._get_float(row['fraction_input'], f"Доля карбоната (строка {i+1})")
                calc_degree = self._get_float(row['calc_degree_input'], f"Степень кальцинирования (строка {i+1})")
