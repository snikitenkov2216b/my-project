...
# Код обновлен для подключения всех вкладок с категориями расчетов.
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, data_service: DataService):
        ...
        # self.setWindowIcon(QIcon('path/to/your/icon.png')) # Раскомментируйте и укажите путь к иконке
    ...
    def _init_ui(self):
        ...
        self.tabs.addTab(Category9Tab(self.data_service), "Кат. 9: Керамика")
        self.tabs.addTab(Category10Tab(self.data_service), "Кат. 10: Производство аммиака")
        self.tabs.addTab(Category11Tab(self.data_service), "Кат. 11: Хим. производство (N2O)")
        self.tabs.addTab(Category12Tab(self.data_service), "Кат. 12: Нефтехимия")
        self.tabs.addTab(Category13Tab(self.data_service), "Кат. 13: Фторсодержащие")
        ...
        self.tabs.addTab(Category19Tab(self.data_service), "Кат. 19: Дор. хозяйство")
        self.tabs.addTab(Category20Tab(self.data_service), "Кат. 20: Захоронение отходов")
        self.tabs.addTab(Category21Tab(self.data_service), "Кат. 21: Био. переработка")
        self.tabs.addTab(Category22Tab(self.data_service), "Кат. 22: Сжигание отходов")
        self.tabs.addTab(Category23Tab(self.data_service), "Кат. 23: Сточные воды (CH4)")
        self.tabs.addTab(Category24Tab(self.data_service), "Кат. 24: Сточные воды (N2O)")

        # Здесь будут добавлены вкладки для категорий ПОГЛОЩЕНИЙ