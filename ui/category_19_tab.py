...
class Category19Tab(QWidget):
    ...
    def __init__(self, calculator: Category19Calculator, parent=None):
        super().__init__(parent)
        self.calculator = calculator
        self.fuel_rows, self.road_work_rows = [], []
    ...
    def _add_fuel_row(self):
        ...
        remove_button.clicked.connect(
            lambda: self._remove_row(row_data, self.fuels_layout, self.fuel_rows)
        )
    ...
    def _add_road_work_row(self):
        ...
        type_combo.addItems(self.calculator.data_service.get_road_types_table_19_1())
        stage_combo = QComboBox()
        stage_combo.addItems(self.calculator.data_service.get_road_stages_table_19_1())
        category_combo = QComboBox()
        category_combo.addItems(
            list(
                self.calculator.data_service.table_19_1[
        ...
        years_input.setToolTip(
            "Срок, на который рассчитаны работы (для распределения выбросов по годам)."
        )
        remove_button = QPushButton("Удалить")
        ...
        self.road_works_layout.insertWidget(
            self.road_works_layout.count() - 1, row_widget
        )
        remove_button.clicked.connect(
            lambda: self._remove_row(
                row_data, self.road_works_layout, self.road_work_rows
            )
        )
                ...
                fuel_data = [
                    {
                        ...
                        "consumption": self._get_float(
                            r["consumption"], f"Расход для '{r['combo'].currentText()}'"
                        ),
                    }
                    for r in self.fuel_rows
                ]
                co2_emissions = self.calculator.calculate_from_energy_consumption(
                    fuel_data
                )
                ...
                road_works_data = [
                    {
                        ...
                        "length": self._get_float(r["length"], f"Протяженность"),
                        "years": r["years"].value(),
                    }
                    for r in self.road_work_rows
                ]
                co2_emissions = self.calculator.calculate_from_road_length(
                    road_works_data
                )
                self.result_label.setText(
                    f"Результат: {co2_emissions:.4f} тонн CO2 в год"
                )

            logging.info(f"Category 19 calculation successful: CO2={co2_emissions}")
        except ValueError as e:
            logging.error(f"Category 19 Calculation - ValueError: {e}")
            QMessageBox.warning(self, "Ошибка ввода", str(e))
            self.result_label.setText("Результат: Ошибка")
        except Exception as e:
            logging.critical(
                f"Category 19 Calculation - Unexpected error: {e}", exc_info=True
            )
